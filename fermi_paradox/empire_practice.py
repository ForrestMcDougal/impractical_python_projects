"""Build a 2-D model of galaxy, post expansion rings for galactic empire."""
...
def model_expansion():
    ...
    c.create_text(-475, -350, anchor="w", fill="red", text=f"Increment = {increment:,}")
    c.create_text(
        -475,
        -325,
        anchor="w",
        fill="red",
        text=f"Velocity as fraction of Light = {SPEED:,}",
    )
    ...
    for years in range(increment, MAX_YEARS + 1, increment):
        ...
        c.create_text(
            -475, text_y_loc, anchor="w", fill="red", text=f"Years = {years:,}"
        )
        text_y_loc += 20
