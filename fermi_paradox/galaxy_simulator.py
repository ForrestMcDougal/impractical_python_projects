...
c = tk.Canvas(root, width=1_000, height=800, bg="black")
c.grid()
...
def spirals(b, r, rot_fac, fuz_fac, arm):
    ...
    for i in range(theta_max_degrees):  # range(0, 600, 2) for no black holes
        theta = math.radians(i)
        x = (
            r * math.exp(b * theta) * math.cos(theta + math.pi * rot_fac)
            + randint(-fuzz, fuzz) * fuz_fac
        )
        y = (
            r * math.exp(b * theta) * math.sin(theta + math.pi * rot_fac)
            + randint(-fuzz, fuzz) * fuz_fac
        )
...
def main():
    ...
    # display legend
    c.create_text(-455, -360, fill="white", anchor="w", text=f"One Pixel = {SCALE} LY")
    c.create_text(
        -455, -330, fill="white", anchor="w", text=f"Radio Bubble Diameter = {SCALE} LY"
    )
    c.create_text(
        -455,
        -300,
    ...
    if SCALE == 225:
        ...
        c.create_text(
            118, 72, fill="red", anchor="w", text="<---------- Earth's Radio Bubble"
        )
