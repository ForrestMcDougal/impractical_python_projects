...
def montecarlo(returns):
    ...
    while case_count < int(num_cases):
        ...
        duration = int(
            random.triangular(int(min_years), int(max_years), int(most_likely_years))
        )
...
def main():
    ...
    plotdata = outcome[:3_000]  # only plot first 3,000 runs

    plt.figure(
        f"Outcome by case (showing first {len(plotdata)} runs", figsize=(16, 5)
    ...
    plt.title(
        f"Probability of running out of money = {odds}%", fontsize=20, color="red"
    )
    plt.show()
